# [level 0] 대소문자 바꿔서 출력하기 - 181949 

[문제 링크](https://school.programmers.co.kr/learn/courses/30/lessons/181949) 

### 성능 요약

메모리: 4.12 MB, 시간: 2.71 ms

### 구분

코딩테스트 연습 > 코딩 기초 트레이닝

### 채점결과

정확성: 100.0<br/>합계: 100.0 / 100.0

### 제출 일자

2026년 02월 01일 17:04:35

### 문제 설명

<p>영어 알파벳으로 이루어진 문자열 <code>str</code>이 주어집니다. 각 알파벳을 대문자는 소문자로 소문자는 대문자로 변환해서 출력하는 코드를 작성해 보세요.</p>

<hr>

<h5>제한사항</h5>

<ul>
<li>1 ≤ <code>str</code>의 길이 ≤ 20

<ul>
<li><code>str</code>은 알파벳으로 이루어진 문자열입니다.</li>
</ul></li>
</ul>

<hr>

<h5>입출력 예</h5>

<p>입력 #1</p>
<div class="highlight"><pre class="codehilite"><code>aBcDeFg
</code></pre></div>
<p>출력 #1</p>
<div class="highlight"><pre class="codehilite"><code>AbCdEfG
</code></pre></div>
<hr>

### 오답 정리

‘\0’ 이 null 문자라는 것을 알아야 풀 수 있는 문제였음.

scanf로 받은 문자열 s1을 반복문으로 1,2,3… 자릿수를 받고 이 반복문이 종료되는 시점은 문자열의 맨 끝에 있는 ‘\0’ null문자를 만나는 시점임. 이 반복문이 키포인트.

반복문을 진행하며 문자열 안에 있는 각각 자리에 있는 문자를 검토하며 아스키 코드표에서 대문자 아스키 65 ~ 90 소문자 아스키 97 ~ 122 범위를 설정하면 된다.

범위에 알맞는 조건문에서 대 —> 소는 s1[a] + 32,  소 —> 대는 s1[a] - 32를 해주고 출력해주면 끝.



<p>※2023년 05월 03일 제한사항이 수정되었습니다.</p>


> 출처: 프로그래머스 코딩 테스트 연습, https://school.programmers.co.kr/learn/challenges
